...
def get_total_consumption(config_file_data: dict, output_file: str = "results/user_data.json", interval: int = 300, minutes: bool = True, start_date: datetime = None, end_date: datetime = None):
    ...
    if result:
        ...
        print("\033[92mSimulation completed correctly\033[0m")
...
def get_battery_data(battery_capacity_ah: float, voltage: float, solar_prod: dict, total_consumpt: dict, charge_eff: float, discharge_eff: float, energy_loss_convrt: float, degrading_ratio: float, initial_state_charge: float = 100.0):
...
def generate_output(houseID, solar_production, electricity_consumption, water_consumption, device_consumption, solar_grid_consumption, battery_data, device_statistical_data, temperature, humidity, air_quality, air_quality_description):
    ...
    solar_production_std = {standardize_timestamp_format(ts): value for ts, value in solar_production.items()}
    electricity_consumption_std = {standardize_timestamp_format(ts): value for ts, value in electricity_consumption.items()}
    water_consumption_std = {standardize_timestamp_format(ts): value for ts, value in water_consumption.items()}
    device_consumption_std = {standardize_timestamp_format(ts): value for ts, value in device_consumption.items()}
    solar_grid_consumption_std = {standardize_timestamp_format(ts): value for ts, value in solar_grid_consumption.items()}
    battery_data_std = {standardize_timestamp_format(ts): value for ts, value in battery_data.items()}
    device_statistical_data_std = {standardize_timestamp_format(ts): value for ts, value in device_statistical_data.items()}
        ...
        ts_data = {
            ...
            "energy_management_sensors": {
                ...
                "energy_efficiency": {
                    ...
                    "load_balancing": device_statistical_data_std[ts]["electricity"]["gini_coefficient"]
                                      if "electricity" in device_statistical_data_std[ts] else 0.0,
    ...
    #Genereate new folder to save the outputs file
...
def complete_simulation_generate(name_of_config_file: str = "mpsds_generate_simulation/config_default.json"):
    ...
    generate_output(
        ...
        water_consumption=total_consumption[1],        # Pass total water consumption
