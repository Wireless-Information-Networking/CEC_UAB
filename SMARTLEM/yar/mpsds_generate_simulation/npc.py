...
# Create a JSON file with the initial structure if it does not exist. This JSON file stores the data generated by the NPC.
...
class NPC:
    def __init__(self, name, out_of_home_periods, house, age_group):
        self.name = name
        self.age_group = age_group
        self.out_of_home_periods = out_of_home_periods
        self.house = house  # Reference to the house
        self.current_room = "Living Room"
        self.state = "Idle"
        self.action_start_time = None # Time when the current action started
...
def run_simulation(config_data, output_path="results/user_data.json", update_interval=300, actions_in_minutes_flag=True, start_date=None, end_date=None):
    ...
    try:
        if type_of_simulation == "fast_forward":
            ...
            raise ValueError("Invalid simulation type in configuration.")
