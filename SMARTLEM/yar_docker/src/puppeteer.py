...
#This will be created dynamically based on the output_dir parameter
...
def generate_output(houseID, solar_production, electricity_consumption, water_consumption, device_consumption, solar_grid_consumption, battery_data, device_statistical_data, temperature, humidity, air_quality, air_quality_description, output_dir="sim_result"):
    ...
    output_file = os.path.join(output_dir, f"{houseID}_output.json")
...
def complete_simulation_generate(name_of_config_file: str = "config_default.json", output_dir: str = "results"):
    ...
    if type_of_simulation == "fast_forward":
        ...
        while True:
            ...
            print("Getting total consumption data...")
            total_consumption = get_total_consumption(config_file_data=config, output_file=os.path.join(output_dir, "user_data.json"))
            print("\033[92mTotal consumption data obtained correctly\033[0m")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        # Configuration file and output directory provided
        print(f"Using configuration file: {sys.argv[1]}")
        print(f"Output directory: {sys.argv[2]}")
        config_file = sys.argv[1]
        ...
        complete_simulation_generate(config_file, output_dir)
    else:
        print("Usage: python puppeteer.py [config_file] [output_directory]")
        exit(1)
    ...
    npcTest_file_path = os.path.join(output_dir, "user_data.json")  # Use output directory
    npcTest.analyze_simulation(npcTest_file_path)
    npcTest.plot_npc_awake_times(npcTest_file_path)
