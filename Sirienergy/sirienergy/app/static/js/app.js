...
async function getSurplus() {
    ...
    if (!email) {
        surplusCard.innerHTML = "<p class='error'>All fields are required. Please complete the form.</p>";
        return;
    }
    ...
    try {
        ...
        const dataset = hours.map((hour, index) => ({
            ...
            backgroundColor: peakHours.includes(hour) ? 'rgba(255, 0, 0, 0.5)' : 'rgba(14, 245, 64, 0.5)',
            borderColor: peakHours.includes(hour) ? 'rgba(255, 0, 0, 1)' : 'rgba(14, 245, 64, 1)'
        }));
...
async function getAdvice() {
    try {
        ...
        if (!email) {
            adviceCard.innerHTML = "<p class='error'>All fields are required. Please complete the form.</p>";
            return;
        }
