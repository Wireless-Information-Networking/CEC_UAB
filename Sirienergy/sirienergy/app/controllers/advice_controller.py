"""Flask controller blueprint for tips for energy management."""
...
def get_money_advice(
    ...
    try:
        ...
        if Config.DEBUG and MODULE_DEBUG and 1:
            log_message(
                "debug", MODULE_TYPE, MODULE_NAME, "get_money_advice",
                f"Data got from BD: \n {existing_data}"
            )

        if existing_data is None:
            log_message(
                "error", MODULE_TYPE, MODULE_NAME, "get_money_advice",
                "User not registered"
            )
        ...
        log_message(
            "error", MODULE_TYPE, MODULE_NAME, "get_money_advice",
            f"Something went wrong while pulling data from DB: {e}"
        )
        return "error", e
...
def get_co2_advice(
    ...
    try:
        ...
        if Config.DEBUG and MODULE_DEBUG and 1:
            log_message(
                "debug", MODULE_TYPE, MODULE_NAME, "get_co2_advice",
                f"Data got from BD: \n {existing_data}"
            )

        if existing_data is None:
            log_message(
                "error", MODULE_TYPE, MODULE_NAME, "get_co2_advice",
                "User not registered"
            )
            return "error", "User is not registered"
        else:
...
def get_advice():
    ...
    if Config.DEBUG and MODULE_DEBUG and 1:
        log_message(
            "debug", MODULE_TYPE, MODULE_NAME, "get_advice",
            f"Money advice is {money_advice}"
        )
        log_message(
            "debug", MODULE_TYPE, MODULE_NAME, "get_advice",
            f"CO2 advice is {co2_advice}"
        )
...
def get_cons_peaks():
    ...
    if Config.DEBUG and MODULE_DEBUG and 1:
        log_message(
            "debug", MODULE_TYPE, MODULE_NAME, "get_cons_peaks",
    ...
    if Config.DEBUG and MODULE_DEBUG and 1:
        log_message(
            "debug", MODULE_TYPE, MODULE_NAME, "get_cons_peaks",
            f"Consumption mean: {consumption_mean}"
        )
